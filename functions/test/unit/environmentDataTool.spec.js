...
describe('environmentDataTool HTTPS Callable Cloud Function', () => {
  let environmentDataTool
  let configStub
  let adminInitStub
  let functions
  let admin
  ...
  before(() => {
    ...
    environmentDataTool = require(`../../src/environmentDataTool/index`)
      .environmentDataToolRequest
  ...
  it('responds with hello message when sent an empty request', async () => {
    ...
    const [err, response] = await to(environmentDataTool(data, context))
