...
import { throwIfNotFoundInData, snapToItemsArray, GetOptions, getApp } from './utils/firebase'
...
export default class Users {
  ...
  public async get(options?: GetOptions): Promise<User[] | object[]> {
    ...
    return snapToItemsArray(snap, (projectsSnap: firebase.firestore.DocumentData | undefined) => {
      if (projectsSnap) {
        return !projectsSnap.id ? projectsSnap.data() : new User(projectsSnap.id, projectsSnap.data())
      }
    });
  }
}