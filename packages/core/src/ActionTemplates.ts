...
import { GetOptions, snapToItemsArray, getApp, snapToArray } from './utils/firebase'
...
export default class ActionTemplates {
  ...
  constructor(templateData?: object) {
    ...
    this.ref = getApp().firestore().collection(ACTION_TEMPLATES_COLLECTION);
    if (templateData) {
  ...
  public async get(options?: GetOptions): Promise<ActionTemplate[] | object[]> {
    ...
    return snapToItemsArray(snap, (projectsSnap: firebase.firestore.DocumentData | undefined) => {
      if (projectsSnap) {
        return !projectsSnap.id ? projectsSnap.data() : new ActionTemplate(projectsSnap.id, projectsSnap.data())
      }
    });
  }
}